...
class show extends React.Component {
  ...
  handleOk(qid) {
    ...
    fetchHelper.post('/ch/course/courseDetial/PostSectionResult', body).then(json => {
      if (json.status == 1) {
  ...
  getQAByPate(sid, pageindex, pagesize) {
    fetchHelper.get(`/nc/course/courseDetial/getSectionQAByPage/${sid}?pageIndex=${pageindex}&pageSize=${pagesize}`).then(json => {
      if (json.status == 1) {
        message.error(json.message, 1)
        return
      }
  ...
  submitQ() {
    ...
    fetchHelper.post('/ch/course/courseDetial/PostSectionQuestion', body).then(json => {
      if (json.status == 1) {
        message.error(json.message, 1)
        return
      }
  ...
  getOutLine(cid) {
    fetchHelper.get('/nc/course/courseDetial/getOutline/' + cid).then(json => {
      if (json.status == 1) {
        message.error(json.message, 1)
        return
      }
  ...
  // 1.0 判断用户是否有登录
  checkLogin() {
    fetchHelper.get('/nc/common/account/islogin').then(json => {
      if (json.status == 2) {
  ...
  getSectionInfo(sid) {
    fetchHelper.get('/nc/course/courseDetial/getSectionInfo/' + sid).then(json => {
      if (json.status == 1) {
        message.error(json.message, 1)
        return
      }
          ...
          <Row>
            <Col span='19'>
              <div className={css.video_box}>
                <video src={this.state.sectionInfo && this.state.sectionInfo.video_path} controls='controls'></video>
              </div>
            </Col>
            <Col span='5'>
        ...
        <div className={css.article_cont}>
          ...
          */}
                            ...
                            <div className={css.item_right}>
                              ...
                              <Collapse className={css.replay} bordered={false}>
                                <Panel
                                  ...
                                  header={
                                    <Row>
                                      <Col span='20'>{fmtDate(item.add_time)}</Col>
                                      <Col className={css.action_box} span='4'>
                                        <Icon type='message' /> 回答 {item.reply_count}
                                      </Col>
                                    </Row>
                                  }
                                  key={index}>
                                  {item.replyList.map((repitem, index1) => (
                                    <span>
                                      <p className={css.title}>
                                        {' '}
                                        <Badge count={index1 + 1} style={{ backgroundColor: '#52c41a', boxShadow: '0 0 0 1px #d9d9d9 inset' }} />
                  ...
                  <TabPane
                    ...
                    key='2'>
                    <div className={css.tabp}>
                      <a href={this.state.sectionInfo && this.state.sectionInfo.code_path}>章节代码下载</a> |<a href={this.state.sectionInfo && this.state.sectionInfo.notes_path}>章节文档下载</a>
                    </div>
                  </TabPane>
                </Tabs>
              </div>
