            ...
            if (!err) {
                ...
                .then(json=>{
                    if(json.status == 1){
                        ...
                        //  2、跳转到首页  /index 下面这个方法是antdesign提供的方法，第一个是提示内容，第二个是延迟事件，第三个是回调函数
                ...
                <Col span="10" offset="7">
                    ...
                    <Tabs defaultActiveKey="1">
                            ...
                            <Form onSubmit={this.login} className={css.login_form}>
                                ...
                                <FormItem>
                                    {getFieldDecorator('password', {
                                        ...
                                        <Input prefix={<Icon type="lock" style={{ color: 'rgba(0,0,0,.25)' }} />} auto-complete="new-password" type="password" placeholder="请输入密码" />
                                    )}
                                </FormItem>
                                <FormItem>
                        ...
                        <TabPane tab={<span><Icon type="android" />注册</span>} key="2">
                            {/* 使用引入的注册组件，防止在一个组件同时出现两个form */}
                            <Register></Register>
                        </TabPane>
                    </Tabs>
                </Col>
            </Row>
        {/* 如果在style中技术jsx，意思就是针对这个组件的局部样式；<style jsx></style>,里面用模板字符串。不加就是用来覆盖ant-design */}
        <style>{
            `
            .ant-form-item{
                margin:10px 10px;
